 ...
 *
 * rodin r12: 6.6 设备树的 drivers/dma-buf/heaps/page_pool.c 原样搬回
 * （移植到 6.18 时整文件丢了，而 mtk_sec_heap.ko / mtk_heap_refill.ko 仍在用它的
 *  dmabuf_page_pool_* 导出）。唯一改动：6.18 没有
 *  trace_android_vh_dmabuf_page_pool_free_bypass 这个 hook（同名 hook 已不存在），
 *  去掉该回调调用，退回"总是回到池子"的上游行为。
 *  register_shrinker(shrinker, fmt, ...) 的 6.6 两参形式由
 *  lib/compat-6.6-core.c 提供，可直接沿用。
 */
