...
static void gzvm_destroy_vm(struct gzvm *gzvm)
{
	...
	dev_dbg(gzvm->gzvm_drv->dev, "VM-%u is going to be destroyed\n",
		gzvm->vm_id);
...
static int gzvm_create_vm_debugfs(struct gzvm *vm)
{
	...
	if (!vm->gzvm_drv->gzvm_debugfs_dir) {
		dev_warn(vm->gzvm_drv->dev,
			 "VM debugfs directory is not exist\n");
		return -EFAULT;
	}

	if (vm->debug_dir) {
		dev_warn(vm->gzvm_drv->dev,
			 "VM debugfs directory is duplicated\n");
		return 0;
	}

	snprintf(dir_name, sizeof(dir_name), "%d-%d", task_pid_nr(current),
		 vm->vm_id);

	dent = debugfs_lookup(dir_name, vm->gzvm_drv->gzvm_debugfs_dir);
	if (dent) {
		dev_warn(vm->gzvm_drv->dev,
			 "Debugfs directory is duplicated\n");
		return 0;
	}
	...
	vm->debug_dir = dent;
	debugfs_create_file("protected_shared_mem", 0444, dent, vm,
			    &shared_mem_fops);
...
static struct gzvm *gzvm_create_vm(struct gzvm_driver *drv, unsigned long vm_type)
{
	...
	if (ret) {
		dev_err(drv->dev, "Failed to initialize irqfd\n");
		kfree(gzvm);
		return ERR_PTR(ret);
	}
	...
	if (ret) {
		dev_err(drv->dev, "Failed to initialize ioeventfd\n");
		gzvm_vm_irqfd_release(gzvm);
		kfree(gzvm);
		return ERR_PTR(ret);
	}
	...
	if (ret) {
		dev_warn(drv->dev, "Failed to create debugfs for VM-%u\n",
			 gzvm->vm_id);
	}

	dev_dbg(drv->dev, "VM-%u is created\n", gzvm->vm_id);
