...
/* rodin stage2: this header is ported wholesale from the vendor tree
 * (MTK_kernel_device_modules-rodin/include/soc/mediatek/smi.h), replacing the
 * 30-line upstream stub. The upstream stub only fed drivers/iommu/mtk_iommu*.c
 * and drivers/memory/mtk-smi.c, none of which is compiled in this kernel
 * (CONFIG_MTK_SMI/CONFIG_MTK_IOMMU unset); the vendor SMI/IOMMU drivers are
 * built instead and share structs across TUs, so one uniform header is
 * mandatory (mtk-memory-port.h r15 whole-file-port precedent). */
