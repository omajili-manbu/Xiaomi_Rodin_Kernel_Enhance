...
/* 6.6 __napi_alloc_skb == 6.18 napi_alloc_skb */
struct sk_buff *__napi_alloc_skb(struct napi_struct *napi, unsigned int len,
				 gfp_t gfp_mask)
	__alias(napi_alloc_skb);
...
{
	return rhashtable_init_618(ht, params);
}
EXPORT_SYMBOL(rhashtable_init);

int rhltable_init(struct rhltable *hlt, const struct rhashtable_params *params)
{
	return rhltable_init_618(hlt, params);
}
...
{
	return snd_soc_set_runtime_hwparams_618(substream, hw);
}
